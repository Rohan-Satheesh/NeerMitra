...
export default function ScanEffect({ isScanning, onScanComplete, className = '' }: ScanEffectProps) {
  ...
  const detections = [
    { label: 'SST GRADIENT DETECTED', detail: '28.4°C (+0.8°C anomaly)', color: 'text-cyan-400' },
    { label: 'CHLOROPHYLL BLOOM LOCATED', detail: 'High concentration at 11.5°N, 74.1°E', color: 'text-emerald-400' },
    { label: 'AIS VESSEL CLUSTERS SYNCED', detail: '247 vessels tracked in Indian EEZ', color: 'text-blue-400' },
    { label: 'DYNAMIC RISK ENGINE RE-CALCULATED', detail: 'Fleet safety index: 94% Nominal', color: 'text-amber-400' },
  ];
